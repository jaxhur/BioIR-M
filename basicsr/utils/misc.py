    ...
    if opt['path']['resume_state']:
        ...
        for network in networks:
            ...
            if opt['path'].get('ignore_resume_networks') is None or (
                ...
                candidate_names = [
                    f'{resume_iter}_{basename.upper()}.pth',
                    f'net_{basename}_{resume_iter}.pth'
                ]
                candidates = [
                    osp.join(opt['path']['models'], filename)
                    for filename in candidate_names
                ]
                load_path = next(
                    (path for path in candidates if osp.isfile(path)), None)
                if load_path is None:
                    raise FileNotFoundError(
                        f'Resume state iter={resume_iter} was found, but no '
                        f'matching {network} weight exists. Checked: '
                        f'{candidates}')
                opt['path'][name] = load_path
