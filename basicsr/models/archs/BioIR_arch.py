...
class AttModule(nn.Module):
    def __init__(self, dim, bias):
        ...
        self.dim = dim
        self.conv = nn.Conv2d(dim, dim*2, kernel_size=1, bias=bias)
        self.dw = nn.Conv2d(dim*2, dim*2, kernel_size=3, stride=1, padding=1, groups=dim*2, bias=bias)
        self.l2h = L2H(dim)
        self.h2l = H2L(dim)
    ...
    def forward(self, x):
        ...
        x1, x2 = self.dw(self.conv(x)).chunk(2, dim=1)
...
class L2H(nn.Module):
    def __init__(self, dim):
        ...
        self.r = 16
        self.kernel = 3
        self.conv = nn.Conv2d(self.r**2, self.kernel**2, kernel_size=1)
        self.pad = nn.ReflectionPad2d(self.kernel//2)
    ...
    def forward(self, low, high):
        ...
        n, c, h, w = high.shape
        high = F.unfold(self.pad(high), kernel_size=self.kernel).reshape(n, 1, self.kernel**2, h, w)
...
class H2L(nn.Module):
    def __init__(self, dim, kernel_size=5):
        ...
        self.dw = nn.Conv2d(dim, dim, kernel_size=self.dw_k, padding=self.dw_k//2, groups=dim)
        self.cv2 = nn.Conv2d(dim, self.kernel_size ** 2, kernel_size=1)
    ...
    def forward(self, low, high):
        ...
        n, c, h, w = low.shape
        low = torch.mean(low, dim=1, keepdim=True)
        low = F.unfold(self.pad(low), kernel_size=self.kernel_size).reshape(n, 1, self.kernel_size**2, *low.shape[-2:])

        weight = self.cv2(high)
        weight = weight.reshape(n, 1, self.kernel_size**2, *weight.shape[-2:]) # b, k**2, h, w

        out = torch.sum(low * weight, dim=2)
